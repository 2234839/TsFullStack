...
/** 数据库路径从环境变量读取，默认使用 prisma/dev.db */
const DATABASE_PATH =
  process.env.DATABASE_URL ? process.env.DATABASE_URL.replace('file:', '') : './prisma/dev.db';
...
export class PrismaService extends Context.Tag('PrismaService')<
  PrismaService,
  {
    ...
    readonly prisma: ClientContract<typeof schema>;
...
export const PrismaServiceLive = PrismaService.of({
  prisma: new ZenStackClient(schema, {
  ...
  getPrisma: (opt: {
    ...
    Effect.gen(function* () {
      ...
      const authDb = prisma.$use(new PolicyPlugin());
