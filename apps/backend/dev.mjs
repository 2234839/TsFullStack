...
// 防抖定时器：数组配置下多个产物会各自报一次 Rebuilt，需合并处理
let restartTimer = null;
const RESTART_DEBOUNCE_MS = 800;

/** 统一的服务器（重启）逻辑，带防抖 */
function scheduleServerStart(isInitial) {
...
// 启动 vp pack watch，监听编译完成事件（包括首次构建和后续 rebuild）
const tsdown = spawn("vp", ["pack", "--watch"], {
...
tsdown.stdout?.on("data", (data) => {
  ...
  if (
    ...
    output.includes("Rebuilt in") ||
    output.includes("✔ Rebuilt")
  ) {
    if (!initialBuildDone) {
      // 首次构建完成（防抖等待所有产物构建完毕再启动）
      initialBuildDone = true;
      scheduleServerStart(true);
    } else {
      // 后续 rebuild，防抖重启服务器
      scheduleServerStart(false);
    }
  }
});
