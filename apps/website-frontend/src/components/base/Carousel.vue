...
<template>
  <div v-bind="$attrs" class="relative overflow-hidden">
    <!-- 轮播内容 -->
    <div class="flex transition-transform duration-300 gap-2">
      <template v-for="(item, _idx) in visibleItems" :key="_idx">
    ...
    <template v-if="totalSlides > 1">
      <button
        ...
        :aria-label="t('上一张')"
      >
        <i class="pi pi-chevron-left text-primary-body"></i>
      </button>
      ...
      <button
        ...
        :aria-label="t('下一张')"
      >
        <i class="pi pi-chevron-right text-primary-body"></i>
      </button>
    </template>
