<template>
  <Tooltip :content="tooltipText">
    <div @click="toggleThemeMode"
      ...
      <!-- 太阳图标 (dark) -->
      <svg v-if="currentMode === 'dark'" key="dark" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="5"/><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42"/></svg>
      <!-- 随机/刷新图标 (random) -->
      <svg v-else-if="currentMode === 'random'" key="random" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12a9 9 0 0 0-9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M3 12a9 9 0 0 0 9 9 9.75 9.75 0 0 0 6.74-2.74L21 16"/><path d="M16 16h5v5"/></svg>
      <!-- 月亮图标 (light/default) -->
      <svg v-else key="light" xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 12.79A9 9 0 1 1 11.21 3 7 7 0 0 0 21 12.79z"/></svg>
    </div>
  </Tooltip>
</template>
...
<script setup lang="ts">
  ...
  const themeButtonClass = computed(() => {
    switch (currentMode.value) {
      ...
      case 'light':
      default:
        return 'bg-white dark:bg-primary-900 text-primary-900 dark:text-warning-400 hover:bg-primary-50 dark:hover:bg-primary-800';
    }
  });
