<template>
  <div class="payment-config-page p-6 max-w-4xl mx-auto">
    <!-- 页面标题 -->
    <PageHeader icon="pi pi-wallet text-primary-theme" :subtitle="t('配置支付渠道和密钥信息')">
      {{ t('支付配置') }}
    </PageHeader>
    ...
    <template v-else>
      ...
      <Card class="p-6 mb-6">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-primary-label mb-1">{{ t('App ID') }}</label>
            <Input v-model="config.mbd.appId" :placeholder="t('从面包多开发者后台获取')" />
          </div>
          <div>
            <label class="block text-sm font-medium text-primary-label mb-1">{{ t('App Key (签名密钥)') }}</label>
      ...
      <Card class="p-6 mb-6">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-primary-label mb-1">{{ t('User ID') }}</label>
            <Input v-model="config.afdian.userId" :placeholder="t('爱发电创作者 User ID')" />
          </div>
          <div>
            <label class="block text-sm font-medium text-primary-label mb-1">{{ t('API Token') }}</label>
            <Password v-model="config.afdian.apiKey" :placeholder="t('API 调用签名用 Token')" :feedback="false" :toggleMask="undefined" />
          </div>
          <div class="p-3 rounded-lg bg-primary-50 dark:bg-primary-800 text-xs text-primary-theme">
        ...
        <!-- Webhook 测试按钮 -->
        <div class="mt-4 pt-4 border-t border-primary-default">
          <Button
      ...
      <Card class="p-6 mb-6">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-primary-label mb-1">{{ t('微信号') }}</label>
            <Input v-model="config.wechat.accountId" :placeholder="t('用于展示给用户的站长微信号')" />
          </div>
          <div>
            <label class="block text-sm font-medium text-primary-label mb-1">{{ t('微信昵称(可选)') }}</label>
      ...
      <Card class="p-6 mb-6">
        ...
        <div class="space-y-4">
          <div>
            ...
            <p class="text-xs text-primary-theme mt-1">{{ t('默认30分钟，超时未支付自动取消') }}</p>
          </div>
        </div>
      </Card>
...
/** 是否正在测试 Webhook */
const testingWebhook = shallowRef(false);
/** 测试结果 */
