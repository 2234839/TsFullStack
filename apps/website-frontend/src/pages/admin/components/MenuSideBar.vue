<template>
  ...
  >
    ...
    >
      <!-- 顶部Logo区域 -->
      <div
        class="logo-area border-b border-primary-200 dark:border-secondary-700/50"
        :class="[
          isCollapsed
      ...
      <!-- 用户信息区域 -->
      <div
        class="user-profile px-4 py-5 flex items-center border-b border-primary-200 dark:border-secondary-700/50"
      >
        <div class="relative">
          <a href="https://shenzilong.cn" target="_blank">
            <div
              class="shadow-lg border-2 border-primary-400 dark:border-info-400 mr-3 rounded-full"
            >
              <UserAvatar :fileId="avatarFileId" size="md" />
            </div>
          </a>
          <span
            class="absolute bottom-0 right-0 w-3.5 h-3.5 bg-success-500 border-2 border-white dark:border-secondary-900 rounded-full"
          ></span>
        </div>
        <div
        ...
        >
          ...
          <p class="text-sm text-primary-500 dark:text-secondary-400">
            {{ userProfile?.email ?? t("未登录") }}
          </p>
        </div>
      ...
      <!-- 菜单区域 -->
      <div
        class="menu-container flex-1 overflow-y-auto custom-scrollbar px-3 py-2"
        style="--scrollbar-size: 4px; --scrollbar-radius: 20px"
      >
        <div v-if="isCollapsed" class="collapsed-menu">
          <ul class="flex flex-col items-center space-y-1">
            <li v-for="item in collapsedMenuItems" :key="item.key" class="w-full relative">
              <!-- 如果有子菜单，使用 Dropdown -->
              <Dropdown
                v-if="hasSubmenuItems(item)"
                v-model="dropdownOpenStates[item.key]"
                side="right"
                align="start"
                :sideOffset="4"
              >
                <template #trigger>
                  <Button
                    ...
                    :class="[
                      ...
                      isActiveRoute(item) ? 'text-primary-600 dark:text-info-400' : '',
                      'has-submenu',
                    ]"
                    :title="item.label"
                  />
                ...
                <div class="py-1">
                  ...
                  >
                    ...
                    <Badge
                      v-if="subItem.badge"
                      :value="subItem.badge"
                      :variant="getBadgeVariant(subItem)"
                    ></Badge>
                  </div>
                </div>
              </Dropdown>
              ...
              <template v-else>
                ...
                <Badge
                  ...
                  class="absolute top-0 right-0 transform translate-x-1 -translate-y-1 scale-75"
                ></Badge>
              </template>
            </li>
          </ul>
            ...
            <div v-for="menuItem in category.items" :key="menuItem.key" class="menu-item relative">
              ...
              >
                ...
                <Badge
                  ...
                  :variant="getBadgeVariant(menuItem)"
                  class="mr-2"
                >
                </Badge>
                <i
...
:deep(.p-popover-submenu) {
  ...
  box-shadow:
    0 10px 15px -3px color-mix(in srgb, var(--color-gray-900) 10%, transparent),
    0 4px 6px -2px color-mix(in srgb, var(--color-gray-900) 5%, transparent);
...
.grid-overlay {
  ...
  background-image:
    linear-gradient(
      to right,
      color-mix(in srgb, var(--color-slate-500) 5%, transparent) 1px,
      transparent 1px
    ),
    linear-gradient(
      to bottom,
