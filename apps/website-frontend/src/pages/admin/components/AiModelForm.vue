<template>
  <Dialog v-model:open="localVisible" :title="isEdit ? t('编辑AI模型') : t('添加AI模型')">
    <div class="space-y-4">
      <div class="field">
        ...
        />
        <small v-if="errors.name" class="text-danger-500 dark:text-danger-400">{{
          errors.name
        }}</small>
      </div>
      ...
      <div class="field">
        ...
        />
        <small v-if="errors.model" class="text-danger-500 dark:text-danger-400">{{
          errors.model
        }}</small>
      </div>
      ...
      <div class="field">
        ...
        />
        <small v-if="errors.baseUrl" class="text-danger-500 dark:text-danger-400">{{
          errors.baseUrl
        }}</small>
      </div>
      ...
      <div class="field">
        ...
        />
        <small v-if="errors.apiKey" class="text-danger-500 dark:text-danger-400">{{
          errors.apiKey
        }}</small>
      </div>
    ...
    <template #footer>
      <div class="flex justify-end gap-2">
        <Button :label="t('取消')" icon="pi pi-times" variant="text" @click="onCancel" />
        <Button
          :label="t('保存')"
          icon="pi pi-check"
          variant="primary"
...
const validateField = (field: string) => {
  switch (field) {
    ...
    case "baseUrl":
      ...
      break;
    case "apiKey":
      // 编辑模式留空表示保留原密钥（后端不再回显 apiKey，泄露面收敛）
      errors.apiKey = form.apiKey.trim() || isEdit.value ? "" : t("请输入API密钥");
      break;
  }
};
...
const onSubmit = async () => {
  ...
  try {
    ...
    if (isEdit.value && model) {
      /** 编辑模式：apiKey 留空表示保留原密钥，不提交该字段 */
      const submitData = apiKey.trim() ? { ...rest, apiKey } : rest;
