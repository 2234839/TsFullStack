<template>
  <div class="svg-generator p-4 max-w-4xl mx-auto">
    ...
    <div v-if="previewData" class="preview-container p-4 border border-primary-default rounded">
      <img :src="previewData" :alt="t('预览')" />
      <Button @click="generateSVG" class="mt-2">
        {{ t('生成SVG描边') }}
      </Button>
    </div>
    <div v-if="svgContainer?.innerHTML" class="mt-4 p-4 border border-primary-default rounded">
...
<script setup lang="ts">
  ...
  const generateSVG = () => {
    ...
    svgContainer.value.replaceChildren(svgEl);
  };
